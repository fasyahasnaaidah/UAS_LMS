  ...
  const renderItem = ({ item, index }: any) => (
    ...
    >
      <View>
        ...
        <Text style={styles.meetingTopic}>{item.session_topic}</Text>
        <View style={styles.meetingMetaRow}>
          <Ionicons name="calendar-outline" size={14} color="#666" style={styles.meetingMetaIcon} />
          <Text style={styles.meetingDate}>{formatDateFull(item.session_date)}</Text>
        </View>
        <View style={styles.meetingMetaRow}>
...
const styles = StyleSheet.create({
  ...
  meetingCard: { 
    ...
    elevation: 3, shadowOpacity: 0.1, borderWidth: 1, borderColor: '#F0F0F0'
  },
