...
const Bubble = ({ text, time, position, sender }: any) => (
...
export default function ChatDiscussionScreen({ navigation, route }: any) {
  const { groupName, courseId, studentCount } = route.params;
  const groupInitial = String(groupName || '').charAt(0);
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/instructor/discussions/course/${courseId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
...
const styles = StyleSheet.create({
  ...
  senderText: { fontSize: 11, color: '#666', marginBottom: 4 },
